export default function Todo({is, name, deleteTodo, handleSelected, toggleEditTodo, isEdited, setEditedName, editTodo}) {
    const handleEditedName = ({target: {value}}) => {
        setEditedName(value)
    }
    return (
    <div className={is+"-todo-item"}>
        {isEdited ? <input className="editInput" placeholder='수정내용' defaultValue={name} onChange={handleEditedName}/> : <span>{name}</span>}
        <div className={is + "-todo-item-btns"}>
